...
const Navbar = () => {
  ...
  const {setShowSearch,getCartCnt}=useContext(ShopContext);
  ...
  return (
    <div className='flex items-center justify-between py-5 font-medium relative z-50'>
      ...
      {/* Icons (Search, Profile, Cart) - FIXED STRUCTURE */}
      <div className='flex items-center gap-8'> {/* Reduced gap */}
        ...
        <Link to={'/login'}>  <img className='w-5 cursor-pointer' src={assets.profile_icon} alt="" /></Link>
          <div className='group-hover:block hidden absolute dropdown-menu right-0 pt-4 z-50'>
            <div className='flex flex-col gap-2 w-36 px-5 py-3 bg-slate-100 text-gray-500 rounded shadow-md'>
        ...
        <Link to='/cart' className='relative'>
          ...
          <p className='absolute right-[-5px] w-4 text-center leading-4 bg-black text-white aspect-square rounded-full text-[8px]'>{getCartCnt()}</p>
        </Link>
